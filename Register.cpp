...
double Register::calculateDepartTime() {
  ...
  if (queue->get_head() == nullptr) {
    ...
    if (availableTime > queue->get_head()->get_arrivalTime()) {
      weneedmax = availableTime;
    } else {
      weneedmax = queue->get_head()->get_arrivalTime();
    }
    double time = weneedmax +
                  (queue->get_head()->get_numOfItems()) * secPerItem +
                  overheadPerCustomer;
    return time;
  }
}
